...
1. YEAR
   - 如果输入timeString："2024"
    - 假如当前系统时间就是在 2024年，并且月份是9月，则应输出 2024-01~2024-09，也就是说相同年份的结束月份必须取系统当前月份。
    - 假如当前系统时间是在 2022年9月，则应输出 2025-01~2025-12，也就是说不同年份的结束月份必须是12月。
   - timeType 必须降级为 "MONTH"

2. QUARTER
   - 如果输入timeString："2025-Q2"
    - 应输出对应该季度的范围2025-04~2025-06
   - 扩展为该季度的自然月份范围（如 Q1 → 01~03 Q2 → 04~06 Q3 → 07~09 Q4 → 10~12）
   - 降级为 "MONTH"

3. MONTH
   - 如果输入timeString："2023-06"
    - 应输出 2023-06-01~2023-06-30（精确天数你可以自行推算）
   - 扩展为整月的自然天范围，例如："2023-06-01~2023-06-30"
   - 降级为 "DAY"

4. WEEK
   - 如果输入timeString："2025-W15"
   - 扩展为该周的自然日期区间（周一到周日）
   - 降级为 "DAY"

5. TENDAYS（上旬/中旬/下旬）
   - 如果输入timeString："2025-10上旬"
    - 应输出对应的天数范围2025-10-01~2025-10-10
   - 根据自然日进行扩展（如上旬是1~10日，中旬是11~20日，下旬是21~月底）
   - 降级为 "DAY"

6. DAY
   - 如果输入timeString："2024-10-03"
