...
from core.graph_manager import set_graph
from domains.energy.llm.llm_compare_analyzer import call_compare_llm
from domains.energy.ask import reply_templates
from domains.energy.llm.llm_indicator_expander import expand_indicator_candidates
...
async def handle_analysis(
    ...
    for entry in indicators:
        ...
        if not ("~" in entry.get("timeString", "")):
            ...
            parsed_range = await normalize_time_range(entry.get("timeString"), entry.get("timeType"))
    ...
    # -------------------------------------------------------
    logger.info("🟦 所有指标已完成 batch 查询 (%s 个)", len(entries_results))
    from domains.energy.llm.llm_trend_analyzer import call_trend_llm
