# app/domains/energy/llm/llm_energy_indicator_parser.py
import asyncio
import re
...
1. indicator 必须保留原文，包括数字和文字，不要丢失任何信息。
   - 如果数字紧跟在指标词中（如“2030酸轧纯水使用量”、“3030连退纯水使用量”、“3高炉工序能耗实绩报出值”），则视为指标一部分，而非时间。
...
2. timeString 必须根据 timeType 精确格式化：
   ...
   - MONTH → "YYYY-MM"
     ...
     - 如果出现“上个月”假设这个月是2025年10月份），则使用今年年份且前一个月："2025-09"
...
- 只有明确确认描述的是时间区间才使用区间方式，否则一律使用时间点方式
  ...
  - “本月累计的”(假设今天是2024年10月20日) → indicator=null、timeString="2024-10"、timeType="MONTH"
  - "本月计划"(假设今天是2023年9月10日) → indicator=null、timeString="2023-09"、timeType="MONTH"
...
"""
    ...
    # 调用安全 LLM 函数
    result = await core.safe_llm_parse(prompt)
