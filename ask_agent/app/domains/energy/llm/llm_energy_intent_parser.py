# app/domains/energy/llm/llm_energy_intent_parser.py

import logging
from app import core

logger = logging.getLogger("energy.llm.intent_parser")
...
- 指标中可能包含描述性质的后缀词（如“累计”、“计划”、“目标”、“完成值”、“用量”、“指标”、“成本”、“效率”、“总量”、“单耗”、“强度”等），可以放在每条记录最后面。
  ...
  - 输入: "高炉工序能耗本月计划是多少"，输出: {"intent":"single_query","candidates":["本月高炉工序能耗计划"]}。  
...
输入: "本月1、2高炉工序能耗实绩报出值偏差情况"
输出: {"intent":"compare","candidates":["本月1高炉工序能耗实绩报出值","本月2高炉工序能耗实绩报出值"]}
"""
    ...
    async def parse_intent(self, user_input: str) -> dict:
        ...
        try:
            # 调用 LLM
            data = await core.safe_llm_parse(self.INTENT_PROMPT + "\n输入: " + user_input)
            ...
            # ---- candidates fallback ----
            # 情况1：LLM 输出 candidates 不是 list（错误）
            if not isinstance(candidates, list):
                candidates = []

            # 情况2：compare 模式允许 []，不用 fallback
            if intent == "compare":
                pass
